...
const SYSTEM_TAGS = ["не вышел на связь", "чёрный список", "не подходит"];
...
function renderVacancies() {
  ...
  list.forEach(([id, v]) => {
    ...
    card.innerHTML = `
      ...
      <div class="vacancy-card-manager">${escapeHtml(v.manager || "")}</div>
...
async function ensureSystemTags() {
  for (const name of SYSTEM_TAGS) {
    ...
    if (!exists) {
      const colors = { "не вышел на связь": "#c9a24a", "чёрный список": "#333333", "не подходит": "#a85c5c" };
...
$("#saveCandidateBtn").addEventListener("click", async () => {
  ...
  try {
    if (state.currentCandidateId) {
      const prev = state.candidates[state.currentCandidateId];
      await dbUpdate(`candidates/${state.currentCandidateId}`, payload);
      if (prev.vacancyId !== vacancyId) await logHistory(state.currentCandidateId, "Вакансия изменена");
      ...
      const newId = await dbPush("candidates", payload);
...
async function openBulkReturnFlow(defaultVacancyId) {
  ...
  for (const id of state.selectedCandidateIds) {
    await dbUpdate(`candidates/${id}`, { stage: "Отклик", status: "активный", stageChangedAt: nowISO(), employmentDate: null });
...
async function runAutomationChecks() {
  ...
  for (const [id, c] of Object.entries(state.candidates)) {
    ...
    if (c.status === "не вышел на связь") continue;
    const days = daysBetween(c.stageChangedAt, new Date(now).toISOString());
    if (days >= AUTOTRANSFER_DAYS) {
      await dbUpdate(`candidates/${id}`, { status: "не вышел на связь" });
